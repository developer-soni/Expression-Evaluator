...
public class Evaluator {
  ...
  public Evaluator() {
    ...
    Operator.operatorMap.put("+", new AddOperator());
    Operator.operatorMap.put("-", new SubtractOperator());
    ...
    while (this.expressionTokenizer.hasMoreTokens()) {
      if (!(expressionToken = this.expressionTokenizer.nextToken()).equals(" ")) {
        if (Operand.check(expressionToken)) {
          ...
          Operator newOperator = Operator.checkOperatorType(expressionToken);
          ...
          if (expressionToken.equals(")")) {
            ...
            while (!operatorStack.empty() && operatorStack.peek().priority() >= newOperator.priority() && newOperator != Operator.operatorMap.get("(")) {
              ...
              Operand operandTwo = operandStack.pop();
              Operand operandOne = operandStack.pop();
              Operand o = oldOpr.execute(operandOne, operandTwo);
              operandStack.push(o);
            }
